...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 14 18:48:14 2020
...
# Training Phase
        for j in range(0, 70):
            ...
            for k in range(g):            # Splitting the End Cells into Proportions and Updating the Error Equation
                if (k == 0):
                    w_value = (w_value + weight[int(k) + int(q)] * 0.80)
                elif (k == g-1):
                    w_value = (w_value + weight[int(k) + int(q)] * 0.20)
                else:
            ...
            for k in range(g):            # Splitting the End Cells into Proportions and Updating the Error Equation
                if(k == 0):
                    weight[int(k) + int(q)] = (weight[int(k) + int(q)]) + (corrected_val * 0.80)
                elif(k == g-1):
                    weight[int(k) + int(q)] = weight[int(k) + int(q)] + (corrected_val * 0.20)
                else:
    ...
    if g == 3:  # take different values of g 
        w_save = w_rand
        weight_save = weight
